...
\def\ttverbatim{\begingroup\undocatcodespecials \catcode`\#=\active
  \def\par{\ifvmode\allowbreak\vskip 0pt plus 1pt\else\endgraf\nobreak\fi}
...
\def\resetpageno{\ifnum\pageno<0 \pageno=-1 \else \pageno=1 \fi}
...
\outer\long\def\Chapter#1 \par{\vfill\supereject \headlinefalse
  \ifodd\pageno\else\null\eject\headlinefalse\fi
  \advance\chapno by1 \secno=0 \exno=0 \resetpageno \def\chapname{#1}
  \label{chapter:#1}
...
\def\typewriter#1'{\leavevmode{\catcode`.=\active
  \chardef\{=`{ \chardef\}=`} \chardef*=`* \chardef"=`" \chardef~=`~
  \tt #1}}
\catcode``=\active
...
\def\N{{\bf N}} \def\Z{{\bf Z}} \def\Q{{\bf Q}} \def\R{{\bf R}}
...
\outer\def\FrontMatter{\let\thechapter=\emptychapter
...
\outer\def\Index{\bigskip
  ...
  \pageno=-1
  \headlinefalse
  \let\thechapter=\emptychapter \def\label##1{}
