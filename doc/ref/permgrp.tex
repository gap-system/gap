...
{\GAP}. If `<i> ^ $p\ne i$', we say that <i> is _moved_ by~<p>, otherwise
...
of  positive integers. {\GAP} does _not_  require the user to specify the
...
integers,   so all operations   functions  can  be  applied  (see chapter
"Operations!for groups" and  "Operations!for  permutation groups"). These
operations   also give   rise to   special  kinds   of homomorphisms   of
...
`random' (default `1000') &
        ...
        chain will be done non-deterministically, as described in "Random
        methods!for   permutation groups".   The   probability  that  the
        resulting chain is correct will be at least~$`random'/1000$.
...
recursive, _never print a stabilizer chain!_ (Unless you want to exercise
...
`translabels' &
        ...
        chain   conjugation  (see   "ConjugatedSubgroup!for   permutation
        groups").
...
\>ConjugatedSubgroup( <G>, <p> )!{for permutation groups}
\){<G> ^ <p>}
...
\medskip\noindent
_Centralizer:_ $\Pi \mapsto \Pi  \wedge \Pi u$.  Here, $\wedge$ denotes a
...
\medskip\noindent
_Normalizer:_  First, every $g\in N_G(U)$ must  permute the <U>-orbits of
...
{\rm complement\ of\ }aU)$ can be used.
...
\medskip\noindent _Two-closure:_ By definition,   an element of the  full
symmetric group lies in the  two-closure of <G>  if it preserves all  the
orbital  graphs $(a,b)G$ of~<G>, and   the refinements derived from  this
condition for  the normalizer calculation  (where  the orbital graphs may
more  generally be permuted) can be   strengthened for the computation of
the  two-closure.  To check  whether  a   given  element belongs  to  the
two-closure is more difficult, but for a simple primitive group~<G> (with
few exceptions)  it is shown  in~\cite{LPS88} that  an element belongs to
the two-closure of~<G> if and only if it normalizes~<G> and preserves all
orbital graphs.

\medskip\noindent
_Intersection:_      If     $\Delta_i$   is  the       partition     into
...
<p> ) ),  <p> )' where <hom> is  the associated blocks  homomorphism (see
...
the _earns_,  and it can then  be calculated as `Earns( <G>, MovedPoints(
<G>  )    )' (remember that   this  implies  it is  stored   in  <G>, see
"Operations!for  groups"). The  algorithms for  `Earns'  and `Socle'  are
based on  the O'Nan-Scott theorem  on primitive groups and were developed
by   P.~Neumann.  The     theorem  and  the   algorithms  are   described
in~\cite{Neu86}. `Earns' returns `false'  if <G> is not primitive  on~<D>
or not     of  affine  type,   whereas `Socle'   signals   an   error for
non-primitive~<G>.
...
`MAX_SIZE_TRANSVERSAL'}{} (where  size  means length times degree  of the
permutations), they are stored explicitly as lists of permutations, which
makes  it    faster to construct the     transversal elements in $S'.T'$.
...
that $g^p.t^{-1}$ fixes <b>, and  then the stabilizer  of <b> in $\<
H,g>$  is the extension of the  stabilizer $H_b$ by the normalizing
...
first group  $M_0$ in the series of   a solvable <G>  is a _p_oly_c_yclic
_g_enerating _s_ystem for~<G>. It can be obtained by the function `Pcgs'.
...
equal to the list `<pcgs>.generators'.   In fact, the calculation of  the
generating sequence  is a side-effect of  the stabilizer chain extensions
performed according to rules (1) and~(2).

\>ExponentsOfPcElement( <pcgs>, <g> )!{for solvable permutation groups}
\>DepthOfPcElement( <pcgs>, <g> )!{for solvable permutation groups}
\>LeadingExponentOfPcElement( <pcgs>, <g> )!{for solvable permutation
groups}
...
\>RelativeOrderOfPcElement( <pcgs>, <g> )!{for solvable permutation
groups}
...
extension with <g>.  The corresponding values for `<pcgs>.generators' are
stored in <pcgs> as a side-effect of the pcgs calculation.
...
\>NaturalHomomorphismByNormalSubgroup( <G>, <N> )!{for solvable factor
groups!of permutation groups}
...
\>Blocks( <G>, <D> \[, <seed> \] \[, OnPoints \] )!{for permutation
groups}
...
\>GroupGeneralMappingByImages( <P>, <H>, <gens>, <imgs> )!{for
permutation groups}
...
\>CoKernel( <ghom> )!{for `GroupGeneralMappingByImages'!for permutation
groups}
...
\>OperationHomomorphism( {ExternalSet( <P>, <list> )} )!{for
constituents}

For the   construction  of  a  _constituent  homomorphism_ <chom>,  every
...
technique described in  "ConjugatedSubgroup!for permutation groups" (with
...
"ConjugatedSubgroup!for  permutation groups";    the  latter  is  not   a
homomorphism, so the kernel of <chom> is added to the resulting chain).

%				 *  *  *
\stars

\>OperationHomomorphism( {ExternalSet( <P>, <blocks>, OnSets )} )!{for
blocks}

A  _blocks  homomorphism_  <bhom>   in   {\GAP} has  a  record  component
`<bhom>!.reps', which is a list whose  <i>th position the block number of
the point~<i>.

\>ImagesRepresentative( <bhom>, <elm> )!{for blocks homomorphisms}
\>Image( <bhom> \[, <H> \] )!{for blocks homomorphisms}
\>PreImagesRepresentative( <bhom>, <elm> )!{for blocks homomorphisms}
\>PreImage( <bhom>, <H> )!{for blocks homomorphisms}
\>Kernel( <bhom> )!{for blocks homomorphisms}

\textindent{(1)}  The image   of an element    <elm> under <bhom> is  the
permutation   which maps the  point  <i> to  `<bhom>!.reps[ <b>^<elm> ]',
where <b>  is any  point   from the  <i>th block. So    the image can  be
constructed with `PermList' (see "PermList").

\textindent{(2)} To find    the  image of  a subgroup   <H>  (default  is
`<bhom>.source'),  {\GAP} constructs a ``blockwise stabilizer chain''~<S>
of~<H>, which is   internally  represented like  an ordinary   stabilizer
chain; indeed,  at the beginning  <S> is  just a  copy of the  stabilizer
chain of <H>. Then for the first block <B> from <blocks> the current base
point  of <S> is changed  to  `<B>[1]', and  one level  of the stabilizer
chain  of   the  image is    constructed by  the generalized  conjugation
technique  (see   "ConjugatedSubgroup!for permutation     groups"    with
`$map(pnt) = hom$!.reps[<pnt>]' and  <hom> the blocks homomorphism in the
notation of that  section; here   the next  stabilizer  must be  computed
explicitly). The next level `<S>.stabilizer'   is built by enlarging  the
point stabilizer  of `<B>[1]' to  the  block stabilizer  of <B> by adding
transversal elements  from~<S>  which map  `<B>[1]'  to the  other points
of~<B>. This process  is repeated with  <S> replaced  by `<S>.stabilizer'
and <B> the   next  block from  <blocks>.  After  all  blocks  have  been
processed in this way, <S> is a stabilizer chain for  the subgroup of <H>
acting trivially on <blocks>.

\textindent{(3)} Thus, if <H>  is   `<hom>.source', <S> also yields   the
kernel of <hom> after the image has been calculated.

\textindent{(4)} Using the transversals at all levels of  <S>, it is then
also possible to construct a preimage of a given permutation of <blocks>,
i.e., of an element of `<hom>.range'.

\textindent{(5)}   The  preimage  of a    subgroup <I> of  `<bhom>.range'
(default is `<bhom>.range')   is constructed  by  walking the  stabilizer
chain  of <I> upwards from  the trivial  subgroup  (whose preimage is the
kernel of <hom> with stabilizer chain <S>) and adding for every level <T>
a level to <S>: First a base change is performed to get as new base point
the first point in the  ``base block'' of  <T>, even if  this leads to  a
trivial basic orbit of  <S>. Then the new top  level  of <S> is  extended
with a preimage for every element  of `<T>.generators'. Finally, when <T>
has  reached the top  level of the  stabilizer chain of <I>, the modified
<S> is a stabilizer chain for the preimage.

%				 *  *  *
\stars

\>NaturalHomomorphismByNormalSubgroup( <G>, <N> )!{for permutation
groups}
