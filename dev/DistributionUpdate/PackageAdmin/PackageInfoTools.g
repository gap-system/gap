...
UpdatePackageArchives := function(pkgdir, webdir)
  ...
  for pkg in pkgs do
    ...
    if Length(missing) > 0 and Length(missing) < 4 then
      ...
      if Length(dnam) = 0 then
        Print("     could not unpack archive .... SKIPPING !!!\n");
        continue;
      else
        dnam := dnam[1];
      fi;
...
MergePackageArchives := function(pkgdir, tmpdir, archdir, webdir, inmerge)
  local mergedir, pkgs, textfilesmerge, nam, info, d, tf, 
        fname, fun, allfiles, pkg, str, fmt,fn_zooArch;
...
##    od;
  ...
  fn_zooArch := Concatenation("packages-", str, ".zoo");
  Exec(Concatenation("cd ", archdir, "; repack.py ", fn_zooArch, " -all ;" )); 
