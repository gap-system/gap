...
- otherwise those files with one of the following extensions are considered
  ...
  ".bib", ".xml", ".six", ".gap", ".bib", ".tst", ".css", ".html"
...
if os.system('cd '+tmpdir+'; tar cf '+archbase+'.tar *'):
...
if ext <> '.tar.bz2':
  if os.system('rm -f '+arch+'.tar.bz2'):
    ...
    sys.exit(18)
  if os.system('cp '+tmpdir+'/'+archbase+'.tar '+arch+
...
if ext <> '.zoo':
  if os.system('rm -f '+arch+'.zoo'):
    ...
    sys.exit(19)
  if os.system('cd '+tmpdir+'; find * -print | zoo aqIh '+archbase+'.zoo'):
    sys.stderr.write('Cannot create new archive '+archbase+\
                     '.zoo in '+tmpdir+'\n')
    sys.exit(8)
  for fn in textfiles:
    if os.system('cd '+tmpdir+"; (echo '!TEXT!'; echo '/END')|zoo c  "+
                 archbase+'.zoo "'+fn+'" 2> /dev/null'):
      sys.stderr.write('Cannot add comment to file '+tmpdir+'/'+archbase+\
                       '.zoo\n')
      sys.exit(20)
  for fn in binfiles:
    if os.system('cd '+tmpdir+"; (echo '!BINARY!'; echo '/END')|zoo c  "+
                 archbase+'.zoo "'+fn+'" 2> /dev/null'):
      sys.stderr.write('Cannot add comment to file '+tmpdir+'/'+archbase+\
                       '.zoo\n')
      sys.exit(9)
...
if ext <> '-win.zip':
  ...
  if (os.path.getsize(os.path.join(tmpdir, 'binfiles')) > 0 and \
      os.system('cd '+tmpdir+'; cat binfiles | zip -9 -q '+\
      archbase+'-win.zip -@')) or \
     (os.path.getsize(os.path.join(tmpdir, 'textfiles')) > 0 and \
      os.system('cd '+tmpdir+'; cat textfiles | zip -9 -l -q '+\
      archbase+'-win.zip -@')):
