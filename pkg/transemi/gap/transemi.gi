...
##
#H  @(#)$Id: transemi.gi,v 1.11 2002/09/09 11:38:32 gap Exp $
##
...
Revision.monotran_gi :=
    "@(#)$Id: transemi.gi,v 1.11 2002/09/09 11:38:32 gap Exp $";
...
function(S)
		...
		# make sure we have the generators:
		GeneratorsOfSemigroup(S);;

		# sometimes computation of the generators 
		# computes the whole element list (as in ideals)
		if HasAsList(S) then
			return Length(AsList(S));
		fi;
...
function(x, M)
    ...
    if DegreeOfTransformation(x) <> DegreeOfTransformationSemigroup(M) then
		...
		# if the elements of the semigroup are already known, just check them!
		# in case M didn't have a generating set, it was computed in 
		# DegreeOfTransformationSemigroup and may therefore have  a plain list of 
		# elements.
		if HasAsList(M) then
			return x in AsList(M);
		fi;

		# see if the element is one of the generators:
		if x in GeneratorsOfSemigroup(M) then
			return true;	
		fi;
...
function(R)
    ...
    if IsTransformationMonoid(AssociatedSemigroup(R)) then
      gens:= GeneratorsOfMonoid(AssociatedSemigroup(R));
    else
      gens:= GeneratorsOfSemigroup(AssociatedSemigroup(R));
    fi;
...
function( S )
    ...
    #### Not needed  R:= GreensRRelation(S);
...
#    fi;
    ...
    for j in [1..Length(rClasses)] do
        for class in rReps[j] do
            class:= GreensRClassOfElement(S, class);
            SetGenSchutzenbergerGroup(class, 
                GenSchutzenbergerGroup(rClasses[j]));
            SetSchutzImages(class, SchutzImages(rClasses[j]));
            SetSchutzRMults(class, SchutzRMults(rClasses[j]));
...
function(R)
    ...
    classes:= [];
    x:= Representative(R);
    GenSchutzenbergerGroup(R);
    D:= GreensDClassOfElement(S, x);
	Size(D);		# calculate structural information

	for c in SchutzRCosets( D ) do
		for m in SchutzRMults(R) do
			d:= c/m;
			new:= GreensHClassOfElement(S, x*d);
			Add(classes, new);
		od;
	od;
...
function(L)
    ...
    if IsTransformationMonoid(AssociatedSemigroup(L)) then
      gens:= GeneratorsOfMonoid(AssociatedSemigroup(L));
    else
      gens:= GeneratorsOfSemigroup(AssociatedSemigroup(L));
    fi;
...
#####H:= GreensHRelation(AssociatedSemigroup(L));

    # initialize
    classes:= [];  S:= AssociatedSemigroup(L); x:= Representative(L);
    D:= GreensDClassOfElement(S, x);
    Size(D);    # unfold data structure

    grp:= GenSchutzenbergerGroup(L);
    sch:= AsSubgroup(grp, GenSchutzenbergerGroup(
        GreensHClassOfElement(S, x)));
    ...
    for l in SchutzLMults(L) do
        ...
        # loop over cosets.
        for c in cos do
            new:= GreensHClassOfElement(S, d*c);
...
function(H)
    ...
    local S, x;
    S:= AssociatedSemigroup(H); x:= Representative(H);
    SetRClassOfHClass(H, GreensRClassOfElement(S,x));
    SetLClassOfHClass(H, GreensLClassOfElement(S,x));
...
#M  Size( <H> )
##
InstallMethod(Size, "for h class of trans semi", true,
    [IsGreensHClass and IsTransformationCollection], 0,
        H->Size(GenSchutzenbergerGroup(H)));
...
function(D)
    ...
    x:= Representative(D); S:= AssociatedSemigroup(D);
...
function( S )
    ...
    for reps in RCRepsTransSemi(S) do
        repeat
            d:= GreensDClassOfElement(S, reps[1]);
            Add(classes, d);
...
InstallMethod(GreensJClasses, "for transformation semigroup", true,
    [IsTransformationSemigroup], 0,
function( S )
	local
		dcl, x;

	dcl := GreensDClasses(S);
	for x in dcl do
		SetIsGreensJClass(x, true);
	od;
	return dcl;
end);
...
#############################################################################
##
#F  IsRegularSemigroupElement( <S>, <x> ) . . . . . . . .  regularity check.
##
##  A transformation <x> is regular  inside the transformation semigroup
##  <S>, if <x> lies insied a regular D class of <S>.
##
...
##  has size $r$. [LM 3.2]
##
InstallMethod(IsRegularSemigroupElement, "for a trans semi", IsCollsElms,
...
function(S, x)
    ...
    # maybe <x> lies in a group.
    if Size(Set(x![1]{n})) = r then
        return true;
    fi;
    ...
    # if we get here, no cross section has been found
    return false;
end);

#############################################################################
##
#M  DisplayTransformationSemigroup( <S> )
##
InstallMethod(DisplayTransformationSemigroup, "for trans semi", true,
    [IsTransformationSemigroup], 0,
function(S)
    ...
    displayDClass:= function(D)
        local h, sh;
        h:= EquivalenceClassOfElementNC(GreensHRelation(Parent(D)), 
            Representative(D));
        if IsRegularDClass(D) then
            Print("*[H size = ", Size(h),", ",
                Length(SchutzRMults(SchutzRClassInDClass(D))), " L classes, ", 
                Length(SchutzLMults(SchutzLClassInDClass(D)))," R classes]");
        else
            sh:= Size(h);
            Print("[H size = ", sh,", ",  
				Length(SchutzRMults(SchutzRClassInDClass(D))) 
				* Size(GenSchutzenbergerGroup(SchutzRClassInDClass(D)))/sh, 
				" L classes (",
				Length(SchutzRMults(SchutzRClassInDClass(D))),
				" image types), ", 
				Length(SchutzLMults(SchutzLClassInDClass(D)))
				* Size(GenSchutzenbergerGroup(SchutzLClassInDClass(D)))/sh, 
				" R classes (",
				Length(SchutzLMults(SchutzLClassInDClass(D))),
                " kernel types)]");
        fi;
...
##
##  returns an isomorphism to a permutation group of a group H class
##
InstallOtherMethod(IsomorphismPermGroup, "for transformation H class", true,
    [IsGreensHClass and IsTransformationCollection], 0,
function(H)
    ...
    rep:= Representative(H);
    if RankOfTransformation(rep*rep) < RankOfTransformation(rep) then
        Error("H class is not a group");
    fi;

    grp:= GenSchutzenbergerGroup(H);

    fun:= function(x)
        return MappingPermListList(ImageSetOfTransformation(x),
            OnTuples(ImageSetOfTransformation(x), x));
    end;

    return MappingByFunction(H, grp, fun);
end);
