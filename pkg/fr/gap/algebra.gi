...
##
#H   @(#)$Id: algebra.gi,v 1.22 2009/09/25 14:59:18 gap Exp $
##
...
BindGlobal("STRINGSTOLMACHINE@", function(r,arg,creator)
    ...
    SetIsStateClosedFRAlgebra(i,true);
    return i;
end);
...
BindGlobal("NILLITY@", function(x,isnil)
    ...
    if IsEmpty(powx) then return 1; fi;

    x := Remove(powx);
    n := Remove(pown)+1;
    while not IsEmpty(powx) do
        y := x*Remove(powx);
        if IsZero(y) then
            Remove(pown);
        else
...
InstallMethod(IsNilpotentElement, "(FR) for an associative element",
        [IsAssociativeElement],
        x->ISNIL_GENERIC@(x)<>false);

InstallMethod(IsNilpotentElement, "(FR) for an FR element",
        [IsLinearFRElement],
        x->ISNIL_FR@(x)<>false);
#############################################################################
...
InstallMethod(MatrixQuotient, "(FR) for a FR algebra-with-one and a level",
    ...
    if HasAugmentationIdeal(A) then
        SetAugmentationIdeal(Q,Ideal(Q,List(GeneratorsOfIdeal(AugmentationIdeal(A)),x->Activity(x,n))));
    fi;
    return Q;
end);
...
#M View
##
InstallMethod(ViewObj, "(FR) for an FR algebra",
        [IsFRAlgebra],
        function(A)
    local n;
    n := Length(GeneratorsOfAlgebra(A));
    Print("<self-similar algebra on alphabet ", LeftActingDomain(A), "^",
          Dimension(AlphabetOfFRAlgebra(A)), " with ",n," generator");
    if n>1 then Print("s"); fi;
    if HasDimension(A) then Print(", of dimension ",Dimension(A)); fi;
    Print(">");
end);

InstallMethod(ViewObj, "(FR) for an FR algebra with one",
        [IsFRAlgebraWithOne],
        function(A)
    local n;
    n := Length(GeneratorsOfAlgebraWithOne(A));
    Print("<self-similar algebra-with-one on alphabet ",
          LeftActingDomain(A), "^", Dimension(AlphabetOfFRAlgebra(A)),
          " with ",n," generator");
    if n>1 then Print("s"); fi;
    if HasDimension(A) then Print(", of dimension ",Dimension(A)); fi;
    Print(">");
