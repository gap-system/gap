...
##
#H  @(#)$Id: rwspcftl.gi,v 1.1 1999/02/26 11:51:10 werner Exp $
##
...
Revision.rwspcftl_gi :=
    "@(#)$Id: rwspcftl.gi,v 1.1 1999/02/26 11:51:10 werner Exp $";
...
BindGlobal( "FromTheLeftCollector_CompleteConjugate", function( pcp )
    ...
    G := PcpGroupByCollector( pcp );
    gens := GeneratorsOfGroup( G );

    n := pcp![ PC_NUMBER_OF_GENERATORS ];
    for i in [n,n-1..1 ] do
        # See if any of the conjugates with inverses are missing
        missing := false;
        for j in [n,n-1..i+1] do
            if IsBound( pcp![ PC_CONJUGATES ][j][i] ) and 
               not IsBound( pcp![ PC_CONJUGATESINVERSE ][j][i] ) then
                missing := true;
                break;
            fi;
        od;
        if missing then
            ...
            images := CanonicalPcpParallel( images, gens{[i+1..n]} );
                ...
                if IsBound( pcp![ PC_CONJUGATES ][j][i] ) and 
                   not IsBound( pcp![ PC_CONJUGATESINVERSE ][j][i] ) then
                    pcp![ PC_CONJUGATESINVERSE ][j][i] := 
                      WordByVector( Exponents( images[j-i] ) );
                fi;
            od;
        fi;
            
        # now fill in the other missing conjugaterelations
        for j in [n,n-1..i+1] do
            if IsBound( pcp![ PC_CONJUGATES ][j][i] ) then
        ...
        if not IsBound( pcp![ PC_EXPONENTS ][j] ) then
            for i in [j-1,j-2..1] do
                ...
                if not IsBound( pcp![ PC_EXPONENTS ][i] ) then
                    ...
                    if FromTheLeftCollector_ExponentSums( [i,-1], 1, n ) 
                       <> ev1 then
