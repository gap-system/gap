...
DESERIALIZATION_TAG_INT := MakeReadOnly(OBJ_MAP());
...
BindGlobal("InstallTypeSerializationTag", function(type, tag)
  atomic SERIALIZATION_TAG_REGION do
    if TNUM_OBJ(tag) = TNUM_OBJ(1) then
...
BindGlobal("SERIALIZATION_UPDATE_TAGS", function()
  atomic SERIALIZATION_TAG_REGION do
	...
	MakeReadOnly(StructuralCopy(DESERIALIZATION_TAG_INT_NEW));
      SERIALIZATION_TAG :=
	MakeReadOnly(StructuralCopy(SERIALIZATION_TAG_NEW));
      SERIALIZATION_TAGS_NEED_UPDATE := false;
    fi;
  od;
end);
