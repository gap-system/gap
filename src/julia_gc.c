...
static void ReportDeadChild(Bag bag, const char * what) GAP_GC_NOTSAFEPOINT
{
        ...
        for (UInt i = 0; i < n && i < 40; i++) {
            ...
            if (c[i] == bag)
                fprintf(stderr, " [%lu]=DEAD", (unsigned long)i);
                     ...
                     jl_gc_internal_obj_base_ptr(c[i]) == (jl_value_t *)c[i])
    ...
    if (MarkParent && MarkParentTnum == T_FUNCTION) {
        ...
        Obj nm = c[8];
        if (nm && IS_BAG_REF(nm) &&
            jl_gc_internal_obj_base_ptr(nm) == (jl_value_t *)nm &&
            IS_STRING_REP(nm))
...
// Was <frame> pushed by the kernel? Frames of kernel extensions are recorded
// too, but may hold Julia values of any kind, which cannot be told from
// garbage; only kernel frames hold nothing but bags and immediates. Kernel
// sources share the directory of this file.
static BOOL IsKernelFrame(const void * frame) GAP_GC_NOTSAFEPOINT
{
    static const char * dir = __FILE__;
    static size_t       dirlen = 0;
    if (dirlen == 0) {
        const char * slash = strrchr(dir, '/');
        dirlen = slash ? (size_t)(slash - dir + 1) : 0;
    }
    if (LedgerOverflow)
        return TRUE;
    for (UInt i = 0; i < LedgerDepth; i++)
        if (Ledger[i].frame == frame)
            return strncmp(Ledger[i].file, dir, dirlen) == 0;
    return FALSE;
}
    ...
    for (jl_gcframe_t * f = jl_pgcstack; f; f = f->prev, depth++) {
        ...
        for (UInt i = 0; i < n; i++) {
            ...
            if (v == 0)
                continue;
            if (((UInt)v & 3) != 0) {
                // a tagged immediate; Julia's scanner skips them in both kinds
                // of frame (JuliaLang/julia#62889)
                continue;
            }
            if (jl_gc_internal_obj_base_ptr(v) == v) {
                if (jl_typeof(v) == (jl_value_t *)DatatypeGapObj)
                    continue;
...
static void MarkRootBag(Bag bag) GAP_GC_NOTSAFEPOINT
{
    ...
    if (bag && !(IS_BAG_REF(bag) &&
                 jl_gc_internal_obj_base_ptr(bag) == (jl_value_t *)bag)) {
