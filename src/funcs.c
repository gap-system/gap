    ...
    Stat                call )
{
    ...
    if (TLS_MACRO(UserHasQuit) || TLS_MACRO(UserHasQUIT)) /* the procedure must have called
...
**  values bag.  Finally it returns the result from 'TLS_MACRO(ReturnObjStat)'.
**
...
{
    TLS_MACRO(RecursionDepth)++;                                           
    if ( RecursionTrapInterval &&                                
         0 == (TLS_MACRO(RecursionDepth) % RecursionTrapInterval) )
      RecursionDepthTrap();
}
...
#define CHECK_RECURSION_AFTER     TLS_MACRO(RecursionDepth)--;  PROF_OUT_FUNCTION(func);

#define REMEMBER_LOCKSTACK() \
    int			lockSP = TLS_MACRO(lockStackPointer)
#define CLEAR_LOCK_STACK() \
    if (lockSP != TLS_MACRO(lockStackPointer)) \
    ...
    Obj                 func )
{
   ...
   /* remove the link to the calling function, in case this values bag
    ...
    /* return the result                                                   */
      {
        Obj                 returnObjStat;
        returnObjStat = TLS_MACRO(ReturnObjStat);
        TLS_MACRO(ReturnObjStat) = (Obj)0;
    ...
    Obj                 fexp )
{
    ...
    ENVI_FUNC( func ) = TLS_MACRO(CurrLVars);
    /* the 'CHANGED_BAG(TLS_MACRO(CurrLVars))' is needed because it is delayed        */
...
void            ExecBegin ( Obj frame )
{
    ...
    ADDR_OBJ(execState)[1] = TLS_MACRO(ExecState);
    ADDR_OBJ(execState)[2] = TLS_MACRO(CurrLVars);
    /* the 'CHANGED_BAG(TLS_MACRO(CurrLVars))' is needed because it is delayed        */
    CHANGED_BAG( TLS_MACRO(CurrLVars) );
    ADDR_OBJ(execState)[3] = INTOBJ_INT((Int)TLS_MACRO(CurrStat));
    TLS_MACRO(ExecState) = execState;
    ...
    UInt                error )
{
    ...
    if ( ! error ) {
        ...
        assert( TLS_MACRO(CurrStat)  == 0 );

        /* switch back to the old state                                    */
        SET_BRK_CURR_STAT( (Stat)INT_INTOBJ((ADDR_OBJ(TLS_MACRO(ExecState))[3]) ));
        SWITCH_TO_OLD_LVARS( ADDR_OBJ(TLS_MACRO(ExecState))[2] );
        TLS_MACRO(ExecState) = ADDR_OBJ(TLS_MACRO(ExecState))[1];
