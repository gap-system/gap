...
static FORCE_INLINE uint32_t getblock4 ( const uint32_t * p, int i )
{
  return p[i];
}

static FORCE_INLINE uint64_t getblock8 ( const uint64_t * p, int i )
{
  return p[i];
}
...
static FORCE_INLINE uint32_t fmix4 ( uint32_t h )
{
  h ^= h >> 16;
  h *= 0x85ebca6b;
  h ^= h >> 13;
...
static FORCE_INLINE uint64_t fmix8 ( uint64_t k )
{
  k ^= k >> 33;
