...
static inline void PushOffsBody( void ) {
  assert(TLS_MACRO(OffsBodyCount) <= 1023);
  TLS_MACRO(OffsBodyStack)[TLS_MACRO(OffsBodyCount)++] = TLS_MACRO(OffsBody);
}

static inline void PopOffsBody( void ) {
  assert(TLS_MACRO(OffsBodyCount));
  TLS_MACRO(OffsBody) = TLS_MACRO(OffsBodyStack)[--TLS_MACRO(OffsBodyCount)];
}

static void SetupOffsBodyStack() {
  TLS_MACRO(OffsBodyStack) = AllocateMemoryBlock(1024*sizeof(Stat));
}
    ...
    UInt                line)
{
    ...
    stat = TLS_MACRO(OffsBody) + FIRST_STAT_CURR_FUNC;

    /* increase the offset                                                 */
    TLS_MACRO(OffsBody) = stat + ((size+sizeof(Stat)-1) / sizeof(Stat)) * sizeof(Stat);

    /* make certain that the current body bag is large enough              */
    if ( SIZE_BAG(BODY_FUNC(CURR_FUNC)) == 0 ) {
      ResizeBag( BODY_FUNC(CURR_FUNC), TLS_MACRO(OffsBody) + NUMBER_HEADER_ITEMS_BODY*sizeof(Obj) );
        TLS_MACRO(PtrBody) = (Stat*)PTR_BAG( BODY_FUNC(CURR_FUNC) );
    }
    while ( SIZE_BAG(BODY_FUNC(CURR_FUNC)) < TLS_MACRO(OffsBody) + NUMBER_HEADER_ITEMS_BODY*sizeof(Obj)  ) {
        ResizeBag( BODY_FUNC(CURR_FUNC), 2*SIZE_BAG(BODY_FUNC(CURR_FUNC)) );
        TLS_MACRO(PtrBody) = (Stat*)PTR_BAG( BODY_FUNC(CURR_FUNC) );
    }
    setup_gapname(TLS_MACRO(Input));
    
    /* enter type and size                                                 */
    ADDR_STAT(stat)[-1] = ((Stat)TLS_MACRO(Input)->gapnameid << 48) + ((Stat)line << 32) +
...
Stat NewStat (
    ...
    UInt                size)
{
    return NewStatWithLine(type, size, TLS_MACRO(Input)->number);
}
    ...
    UInt                size )
{
    ...
    expr = TLS_MACRO(OffsBody) + FIRST_STAT_CURR_FUNC;

    /* increase the offset                                                 */
    TLS_MACRO(OffsBody) = expr + ((size+sizeof(Expr)-1) / sizeof(Expr)) * sizeof(Expr);

    /* make certain that the current body bag is large enough              */
    if ( SIZE_BAG(BODY_FUNC(CURR_FUNC)) == 0 ) {
        ResizeBag( BODY_FUNC(CURR_FUNC), TLS_MACRO(OffsBody) );
        TLS_MACRO(PtrBody) = (Stat*)PTR_BAG( BODY_FUNC(CURR_FUNC) );
    }
    while ( SIZE_BAG(BODY_FUNC(CURR_FUNC)) < TLS_MACRO(OffsBody) ) {
        ResizeBag( BODY_FUNC(CURR_FUNC), 2*SIZE_BAG(BODY_FUNC(CURR_FUNC)) );
        TLS_MACRO(PtrBody) = (Stat*)PTR_BAG( BODY_FUNC(CURR_FUNC) );
    }

    /* enter type and size                                                 */
    ADDR_EXPR(expr)[-1] = ((Stat)TLS_MACRO(Input)->gapnameid << 48) +
                          ((Stat)TLS_MACRO(Input)->number << 32) +
                          ((Stat)size << 8) + type;
    ...
    Stat                stat )
{
    ...
    assert( TLS_MACRO(StackStat) != 0 );
    assert( 0 <= TLS_MACRO(CountStat) );
    assert( TLS_MACRO(CountStat) <= SIZE_BAG(TLS_MACRO(StackStat))/sizeof(Stat) );
    assert( stat != 0 );

    /* count up and put the statement onto the stack                       */
    if ( TLS_MACRO(CountStat) == SIZE_BAG(TLS_MACRO(StackStat))/sizeof(Stat) ) {
        ResizeBag( TLS_MACRO(StackStat), 2*TLS_MACRO(CountStat)*sizeof(Stat) );
    }
    ((Stat*)PTR_BAG(TLS_MACRO(StackStat)))[TLS_MACRO(CountStat)] = stat;
    TLS_MACRO(CountStat)++;
}
...
Stat PopStat ( void )
{
    ...
    assert( TLS_MACRO(StackStat) != 0 );
    assert( 1 <= TLS_MACRO(CountStat) );
    assert( TLS_MACRO(CountStat) <= SIZE_BAG(TLS_MACRO(StackStat))/sizeof(Stat) );

    /* get the top statement from the stack, and count down                */
    TLS_MACRO(CountStat)--;
    stat = ((Stat*)PTR_BAG(TLS_MACRO(StackStat)))[TLS_MACRO(CountStat)];
...
{
    /* there must be a stack, it must not be underfull or overfull         */
    assert( TLS_MACRO(StackExpr) != 0 );
    assert( 0 <= TLS_MACRO(CountExpr) );
    assert( TLS_MACRO(CountExpr) <= SIZE_BAG(TLS_MACRO(StackExpr))/sizeof(Expr) );
    assert( expr != 0 );

    /* count up and put the expression onto the stack                      */
    if ( TLS_MACRO(CountExpr) == SIZE_BAG(TLS_MACRO(StackExpr))/sizeof(Expr) ) {
        ResizeBag( TLS_MACRO(StackExpr), 2*TLS_MACRO(CountExpr)*sizeof(Expr) );
    }
    ((Expr*)PTR_BAG(TLS_MACRO(StackExpr)))[TLS_MACRO(CountExpr)] = expr;
    TLS_MACRO(CountExpr)++;
}
...
Expr PopExpr ( void )
{
    ...
    /* there must be a stack, it must not be underfull/empty or overfull   */
    assert( TLS_MACRO(StackExpr) != 0 );
    assert( 1 <= TLS_MACRO(CountExpr) );
    assert( TLS_MACRO(CountExpr) <= SIZE_BAG(TLS_MACRO(StackExpr))/sizeof(Expr) );

    /* get the top expression from the stack, and count down               */
    TLS_MACRO(CountExpr)--;
    expr = ((Expr*)PTR_BAG(TLS_MACRO(StackExpr)))[TLS_MACRO(CountExpr)];
...
void CodeBegin ( void )
{
    ...
    assert( TLS_MACRO(CountStat) == 0 );
    assert( TLS_MACRO(CountExpr) == 0 );

    /* remember the current frame                                          */
    TLS_MACRO(CodeLVars) = TLS_MACRO(CurrLVars);

    /* clear the code result bag                                           */
    TLS_MACRO(CodeResult) = 0;
}
    ...
    UInt                error )
{
    ...
    if ( ! error ) {
        ...
        /* we must be back to 'TLS_MACRO(CurrLVars)'                                  */
        assert( TLS_MACRO(CurrLVars) == TLS_MACRO(CodeLVars) );

        /* 'CodeFuncExprEnd' left the function already in 'TLS_MACRO(CodeResult)'     */
    }
    ...
    /* otherwise clean up the mess                                         */
        ...
        /* empty the stacks                                                */
        TLS_MACRO(CountStat) = 0;
        TLS_MACRO(CountExpr) = 0;

        /* go back to the correct frame                                    */
        SWITCH_TO_OLD_LVARS( TLS_MACRO(CodeLVars) );
    }
    ...
    Int                 startLine)
{
    ...
    setup_gapname(TLS_MACRO(Input));
    FILENAME_BODY(body) = TLS_MACRO(Input)->gapname;
    STARTLINE_BODY(body) = INTOBJ_INT(startLine);
    /*    Pr("Coding begin at %s:%d ",(Int)(TLS_MACRO(Input)->name),TLS_MACRO(Input)->number);
          Pr(" Body id %d\n",(Int)(body),0L); */
    TLS_MACRO(OffsBody) = 0;

    /* give it an environment                                              */
    ENVI_FUNC( fexp ) = TLS_MACRO(CurrLVars);
    CHANGED_BAG( fexp );
    MakeHighVars(TLS_MACRO(CurrLVars));
    ...
    UInt                mapsto )
{
    ...
    ResizeBag( BODY_FUNC(fexp), TLS_MACRO(OffsBody)+NUMBER_HEADER_ITEMS_BODY*sizeof(Obj) );
    ENDLINE_BODY(BODY_FUNC(fexp)) = INTOBJ_INT(TLS_MACRO(Input)->number);
    /*    Pr("  finished coding %d at line %d\n",(Int)(BODY_FUNC(fexp)), TLS_MACRO(Input)->number); */

    /* switch back to the previous function                                */
    SWITCH_TO_OLD_LVARS( ENVI_FUNC(fexp) );

    /* restore the remembered offset                                       */
    TLS_MACRO(OffsBody) = BRK_CALL_TO();
    PopOffsBody();
    ...
    if ( TLS_MACRO(CurrLVars) != TLS_MACRO(CodeLVars) ) {
    ...
    /* otherwise, make the function and store it in 'TLS_MACRO(CodeResult)'           */
    else {
        TLS_MACRO(CodeResult) = MakeFunction( fexp );
    }
...
{
    TLS_MACRO(StackStat) = NewBag( T_BODY, 64*sizeof(Stat) );
    TLS_MACRO(StackExpr) = NewBag( T_BODY, 64*sizeof(Expr) );
}
    ...
    StructInitInfo *    module )
{
    ...
    InitGlobalBag( &TLS_MACRO(StackStat), "TLS_MACRO(StackStat)" );
    InitGlobalBag( &TLS_MACRO(StackExpr), "TLS_MACRO(StackExpr)" );
    ...
    StructInitInfo *    module )
{
  ...
  if (TLS_MACRO(CountExpr) || TLS_MACRO(CountStat))
    return 1;
  ...
  for (i = 0; i < SIZE_BAG(TLS_MACRO(StackStat))/sizeof(UInt); i++)
    ADDR_OBJ(TLS_MACRO(StackStat))[i] = (Obj)0;
  for (i = 0; i < SIZE_BAG(TLS_MACRO(StackExpr))/sizeof(UInt); i++)
    ADDR_OBJ(TLS_MACRO(StackExpr))[i] = (Obj)0;
  /* return success                                                      */
  return 0;
}
