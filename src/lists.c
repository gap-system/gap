    ...
    Obj                 list )
{
    ...
    Pr("%2>[ %2>",0L,0L);
    for ( TLS_MACRO(PrintObjIndex)=1; TLS_MACRO(PrintObjIndex)<=LEN_LIST(list); TLS_MACRO(PrintObjIndex)++ ) {
        elm = ELMV0_LIST( list, TLS_MACRO(PrintObjIndex) );
        if ( elm != 0 ) {
            if ( 1 < TLS_MACRO(PrintObjIndex) )  Pr( "%<,%< %2>", 0L, 0L );
            PrintObj( elm );
        }
        else {
            if ( 1 < TLS_MACRO(PrintObjIndex) )  Pr( "%2<,%2>", 0L, 0L );
        }
    }
    Pr(" %4<]",0L,0L);
}
