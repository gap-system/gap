...
#if defined(SYS_IS_MINGW) && !defined(GAP_DISABLE_SUBPROCESS_CODE)

/****************************************************************************
**
**  IOStreams on native Windows, on top of anonymous pipes: standard output
**  and error share one pipe as they share the pty elsewhere. The child
**  does not see a terminal, so it must flush its own output.
*/
...
static Obj FuncCREATE_PTY_IOSTREAM(Obj self, Obj dir, Obj prog, Obj args)
{
    ...
    // distinct handles: a child rearranging its standard error must not
    // close standard output with it
    HANDLE hErrWrite;
...
static Int WinReadFromPipe(UInt s, char * buf, Int maxlen, UInt block)
{
    ...
    while (nread < maxlen) {
        // poll rather than block in ReadFile, to keep GAP responsive
        while (1) {
        ...
        // native children emit \r\n where a pty delivers the bytes verbatim:
        // squash \r\n to \n, keep a lone \r, and hold back a chunk-final \r
        // until its successor is known
        if (ws->crlast) {
            ws->crlast = FALSE;
            if (next != '\n') {
                buf[nread++] = '\r';
        ...
        nread += out;
        // a chunk of just a held back \r yields nothing: keep waiting
    }
    return nread;
}
...
static UInt syWinQuoteArg(char * dst, UInt pos, const char * arg)
{
    ...
    // MSYS2 programs also take ' as a quote, and expand ~ and globs in
    // unquoted arguments
    if (*arg != '\0' && *arg != '~' &&
        strpbrk(arg, " \t\"'*?[{}()") == NULL) {
        // no quoting needed
...
// append <arg> for cmd.exe, which has no escapes: `/C "cp "a b" c"` runs
// `cp "a b" c`
static UInt syWinQuoteCmdArg(char * dst, UInt pos, const char * arg)
{
    BOOL quote = *arg == '\0' || strpbrk(arg, " \t") != NULL;
...
static BOOL syWinIsCmd(const char * prg)
{
    const char * base = prg;
    ...
    return _stricmp(base, "cmd.exe") == 0;
}
...
SyExecuteProcess(Char * dir, Char * prg, Int in, Int out, Char * args[])
{
    ...
    if (cmdline == NULL)
        return 255;
    // program name quoted, with backslashes: cmd.exe parses its own command
    // line and takes a forward slash path for switches
    pos = 0;
    cmdline[pos++] = '"';
    ...
    BOOL iscmd = syWinIsCmd(prg);
    for (i = 1; args[i] != NULL; i++) {
        cmdline[pos++] = ' ';
        pos = iscmd ? syWinQuoteCmdArg(cmdline, pos, args[i])
                    : syWinQuoteArg(cmdline, pos, args[i]);
    }
    cmdline[pos] = '\0';
