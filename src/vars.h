...
*/
#define CURR_FUNC       (TLS_MACRO(PtrLVars)[0])
...
static inline void SetBrkCallTo( Expr expr, char * file, int line ) {
  if (STEVES_TRACING == True) {
    fprintf(stderr,"SBCT: %i %x %s %i\n",
            (int)expr, (int)TLS_MACRO(CurrLVars), file, line);
  }
  (TLS_MACRO(PtrLVars)[1] = (Obj)(Int)(expr));
}

#else
#define SetBrkCallTo(expr, file, line)  (TLS_MACRO(PtrLVars)[1] = (Obj)(Int)(expr))
#endif

#ifndef NO_BRK_CALLS
#define BRK_CALL_TO()                   ((Expr)(Int)(TLS_MACRO(PtrLVars)[1]))
...
*/
#ifndef NO_BRK_CALLS
#define BRK_CALL_FROM()                 (TLS_MACRO(PtrLVars)[2])
#define SET_BRK_CALL_FROM(lvars)        (TLS_MACRO(PtrLVars)[2] = (lvars))
#endif
#ifdef  NO_BRK_CALLS
...
{
  Obj old = TLS_MACRO(CurrLVars);
  CHANGED_BAG( old );
  TLS_MACRO(CurrLVars) = NewLVarsBag( narg+nloc );
  ...
  if (STEVES_TRACING == True) {
    ...
    fprintf(stderr,"STNL: %s %i\n   func %lx narg %i nloc %i function name %s\n     old lvars %lx new lvars %lx\n",
            file, line, (UInt) func, (int)narg, (int)nloc,s,(UInt)old, (UInt)TLS_MACRO(CurrLVars));
  }
#endif
  return old;
}
...
)
{
  ...
  if (STEVES_TRACING == True) {
    fprintf(stderr,"STOL:  %s %i old lvars %lx new lvars %lx\n",
           file, line, (UInt)TLS_MACRO(CurrLVars),(UInt)old);
  }
#endif
  CHANGED_BAG( TLS_MACRO(CurrLVars) );
  TLS_MACRO(CurrLVars) = (old);
  TLS_MACRO(PtrLVars)  = PTR_BAG( TLS_MACRO(CurrLVars) );
  TLS_MACRO(PtrBody) = (Stat*)PTR_BAG(BODY_FUNC(CURR_FUNC));
}
...
#endif
  CHANGED_BAG( TLS_MACRO(CurrLVars) );
  if (TLS_MACRO(CurrLVars) != old && TNUM_OBJ(TLS_MACRO(CurrLVars)) == T_LVARS)
    FreeLVarsBag(TLS_MACRO(CurrLVars));
  TLS_MACRO(CurrLVars) = (old);
  TLS_MACRO(PtrLVars)  = PTR_BAG( TLS_MACRO(CurrLVars) );
  TLS_MACRO(PtrBody) = (Stat*)PTR_BAG(BODY_FUNC(CURR_FUNC));
}
...
*/
#define ASS_LVAR(lvar,val)      (TLS_MACRO(PtrLVars)[(lvar)+2] = (val))
...
*/
#define OBJ_LVAR(lvar)          (TLS_MACRO(PtrLVars)[(lvar)+2])
