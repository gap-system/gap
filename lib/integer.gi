...
##
InstallMethod( BasisOfDomain, true, [ IsIntegers ], 0,
    CanonicalBasis );
...
InstallMethod( \[\], true, [ IsIntegersEnumerator, IsPosRat and IsInt ], 0,
    function( e, n )
    if n mod 2 = 0 then
      return n / 2;
    else
...
InstallMethod( Position, true, [ IsIntegersEnumerator, IsCyc, IsZeroCyc ], 0,
    function( e, x, zero )
    if not IsInt( x ) then
      return fail;
    elif 0 < x then
...
##
InstallMethod( EuclideanDegree, true, [ IsIntegers, IsInt ], 0,
    function ( Integers, n )
    if n < 0  then
        return -n;
    else
...
##
InstallMethod( EuclideanQuotient, true, [ IsIntegers, IsInt, IsInt ], 0,
...
##
InstallMethod( EuclideanRemainder, true, [ IsIntegers, IsInt, IsInt ], 0,
    function ( Integers, n, m )
    return RemInt( n, m );
    end );
...
##
InstallMethod( Factors, true, [ IsIntegers, IsInt ], 0,
    function ( Integers, n )
    return FactorsInt( n );
    end );
...
function( list )
    ...
    if not ForAll( list, IsInt )  then
        TryNextMethod();
    fi;
    gcd := list[1];
    for i  in [ 2 .. Length(list) ]  do
        gcd := GcdInt( gcd, list[i] );
    od;
    return gcd;
end );
...
#############################################################################
##
#M  Gcd( Integers, <n>, <m> ) . . . . . . . . . . . . . . gcd of two integers
##
InstallMethod( Gcd, true, [ IsIntegers, IsInt, IsInt ], 0,
    function ( Integers, n, m )
    return GcdInt( n, m );
    end );
...
##
InstallMethod( IsIrreducible, true, [ IsIntegers, IsInt ], 0,
    function ( Integers, n )
    return IsPrimeInt( n );
    end );
...
##
InstallMethod( IsPrime, true, [ IsIntegers, IsInt ], 0,
    function ( Integers, n )
    return IsPrimeInt( n );
    end );
...
InstallMethod( IsDoneIterator, true, [ IsIntegersIterator ], 0,
    ReturnFalse );

InstallMethod( NextIterator, true, [ IsIntegersIterator ], 0,
    function( iter )
...
#############################################################################
##
#M  Lcm( Integers, <n>, <m> ) . . . . . . . least common multiple of integers
##
InstallMethod( Lcm, true, [ IsIntegers, IsInt, IsInt ], 0,
    function ( Integers, n, m )
    return LcmInt( n, m );
    end );
...
#M  Log( <n>, <base> )
##
InstallMethod( Log, true, [ IsInt, IsInt ], 0, LogInt );
...
##
InstallMethod( PowerMod, true, [ IsIntegers, IsInt, IsInt, IsInt ], 0,
...
##
InstallMethod( Quotient, true, [ IsIntegers, IsInt, IsInt ], 0,
    function ( Integers, n, m )
    local   q;
    q := QuoInt( n, m );
    if n <> q * m  then
...
##
InstallMethod( QuotientMod, true, [ IsIntegers, IsInt, IsInt, IsInt ], 0,
...
##
InstallMethod( QuotientRemainder, true, [ IsIntegers, IsInt, IsInt ], 0,
    function ( Integers, n, m )
...
InstallMethod( Random, true, [ IsIntegers ], 0,
    function( Integers )
...
##
InstallMethod( RingByGenerators, true,
    [ IsCyclotomicsCollection ], SUM_FLAGS,
    function ( elms )
    if ForAll( elms, IsInt ) then
      return Integers;
...
#M  Root( <n>, <k> )
##
InstallMethod( Root, true, [ IsInt, IsInt ], 0, RootInt );
...
##
InstallMethod( StandardAssociate, true, [ IsIntegers, IsInt ], 0,
    function ( Integers, n )
    if n < 0  then
        return -n;
    else
...
##
InstallMethod( \in, IsElmsColls, [ IsInt, IsIntegers ], 0, ReturnTrue );
