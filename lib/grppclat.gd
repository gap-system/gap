...
##  automorphism in <morph>. If a normal subgroup <N> of <G> is given, this
##  series is chosen to contain <N>.
##
...
##  <G> as a module for the algebra generated by <homs>, this are all
##  submodules.
...
##
##  This function computes subgroups of a solvable group <G>, using the
##  homomorphism principle. It returns a list of representatives up to
##  <G>-conjugacy.
...
##  `actions'. This can be given in the component `funcnorm'. (Otherwise it is
##  computed.)
##  
...
##  function of five parameters, <C>,<A>,<N>,<B>,<M>, that are interpreted as
##  follows: The arguments are groups in the relation $C>A>N>B>M$. $N$ and $M$
##  are normal subgroups. <C> is the full preimage if the normalizer of
##  <A>/<N> in <G>/<N>. When computing modulo <M> and looking for
##  subgroups <U> such that $U\cap N=B$ and $\langle U,N\rangle=A$, this
##  function is called. If it returns 'false' all potential groups <U> (and
##  therefore all groups later arising from them) are disregarded. This can be
##  used for example to compute only subgroups of certain sizes.
##  The actual groups <C>, <A>, <N> and <B> passed to this function will be
##  subgroups of the factor <G>/<M>.
##
##  (This is just a restriction to speed up computations. The function may
##  still return (invariant) subgroups which don't fulfill this property!)
##  This parameter is used to permit calculations of some subgroups if the set
##  of all subgroups would be too large to handle.
##  
...
##  the option `consider' to compute subgroups whose size is divisible by
##  <size>
##
...
##  This function returns a function <consider> of four arguments that can be
##  used in `SubgroupsSolvableGroup' (see "SubgroupsSolvableGroup") for
##  the option `consider' to compute subgroups whose size is exactly
##  <size>
##
