...
## 
InstallMethod(CayleyGraphSemigroup, "for generic finite semigroups", true, 
...
InstallMethod(CayleyGraphDualSemigroup, "for generic finite semigroups", true,
        [IsSemigroup and IsFinite], 0,
    function(s)
        local gens, cg, elms, i;
...
InstallMethod( Enumerator, "for a generic semigroup", true,
    ...
    function(s)
        ...
        enumdata:= rec(
            currentlist:= ShallowCopy(AsSet(GeneratorsOfSemigroup(s))),
            gens:= AsSet(GeneratorsOfSemigroup(s)),
            nextelm:= 1,
            orderedlist:= ShallowCopy(AsSet(GeneratorsOfSemigroup(s)))
        );
...
##
InstallMethod(IsSimpleSemigroup, 
        "for semigroup with generators", true,
        [ IsSemigroup and HasGeneratorsOfSemigroup], 0,
    function(s)
