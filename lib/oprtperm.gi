...
local invrep,  # function: word mapping a point of level <lev> back to <pnt>
  ...
  local n,     # normalizer of <s> in cs[i]
    ...
    od;
    Print("try upstep ",i,": ",Size(q),"; ",Length(zup)," zuppos\n");
    ...
    od;
    Print("Found extra ",Size(u)/Size(s),"\n");
    return u;
  end;
  ...
  for i in [2..Position(cs,b)] do
    Print("Step ",i,": ",Length(orb)," Stb: ",Size(stb),
    ...
    if HasAbelianFactorGroup(cs[i],cs[i-1]) then
      ...
      Add(levgens,[a]);
      Add(levacts,[b]);
      free:=FreeGroup(1); # reuse free groups
      Add(frees,free);
      img:=act(pnt,b);
      ...
      while p<=Length(orb) do
        ...
        if Length(orb)*Size(stb)=Size(cs[i]) then
          Print("early break ",QuoInt(p,leno)+1," of ",Length(lreps),"\n");
          p:=Length(orb)+1;
        fi;
      od;
