...
InstallMethod( BlocksOp, "permgroup on integers",
    ...
    repeat
        ...
        od;
        Info( InfoAction, 4, "BlocksNoSeed ",
        ...
        if Length( block ) = Length( orbit )  then
            Info( InfoAction, 4, "BlocksNoSeed <G> is primitive" );
            return Immutable( [ D ] );
        fi;
...
local   blocks,   # block system of <G>, result
  ...
  for start in orbit{[2..Length(D)]} do
    ...
    repeat
      ...
      od;
      Info(InfoAction,4,"RepresentativesMinimalBlocks ",
               "number of orbits of <H> < <G>_1 is ",nrorbs);
      ...
      # quick test to see if the group is primitive
      if Length( block ) = Length( orbit )  then
        Info(InfoAction,4,"RepresentativesMinimalBlocks <G> is primitive");
        return Immutable([ D ]);
      fi;
...
############################################################################
##
#F  SubnormalOrbitExtension( <G>, <dom>, <pnt>, <gens>, <acts>, <act> )
##
##  <SubnormalOrbitExtension> computes the stabilizer of the point <pnt>
##  under the action <act> of the finite group <G>, where the action is
##  given in a nonstandard way: the group generators <gens> of <G> act as
##  the corresponding elements of the list <acts> (of the same length), i.e.
##  the actual acting objects are the images under the homomorphism
##  <gens>->:<acts>. If <gens>=<acts> no such translation takes place. <dom>
##  is a domain (or a sample list of points) that is used only to create a
##  suitable dictionary for the points in the orbit; <pnt> must be an
##  element of the same kind.
##
##  The orbit and the stabilizer are built up along a composition series of
##  <G>, from the bottom up: for each composition factor the orbit of the
##  subgroup reached so far is extended by the new generator(s) and the
##  Schreier generators arising from collisions are added to the stabilizer.
##  Because every subgroup in the series is normal in the next one, the
##  orbit of the smaller group decomposes the orbit of the larger one into
##  blocks of equal length; this block structure is used to extend the orbit
##  in blocks (avoiding one application of <act> per point and generator)
##  and to keep, for every level, only one word (a `representative') per
##  block. These block representatives are stored as words in free groups,
##  one per level, so that they can be evaluated both in <G> (to build the
##  stabilizer) and in the acting group (to trace points back), see the
##  subfunction `invrep'.
##
##  Only the part of the series below the subgroup generated by the radical
##  and the perfect residuum is treated this way. If the orbit becomes large
##  in comparison to the index of the current subgroup, the subfunction
##  `tryup' attempts to determine the stabilizer instead by extending it
##  upwards by normalizing elements of prime power order (`zuppos'), which
##  can be substantially cheaper.
##
##  The function returns the full stabilizer of <pnt> in <G>.
##
BindGlobal("SubnormalOrbitExtension",function(G,dom,pnt,gens,acts,act)
local invrep,  # function: word mapping a point of level <lev> back to <pnt>
      cs,      # composition series of G, in increasing order
      d,       # dictionary: point -> its position in `orb'
      orb,     # orbit of <pnt> computed so far
      lens,    # lens[i]: length of the orbit after level i
      rlens,   # rlens[i]: orbit growth factor (number of blocks) at level i
      levgens, # levgens[i]: new generators of cs[i] (modulo cs[i-1])
      levacts, # levacts[i]: the images of levgens[i] under `hom' (actors)
      stb,     # stabilizer of <pnt> in the subgroup treated so far
      ablev,   # ablev[i]: is the i-th composition factor abelian?
      frees,   # frees[i]: free group on the generators levgens[i]
      i,       # loop variable: index of the current level in `cs'
      j,       # loop variable (generator index/orbit position)
      k,       # loop variable, also Schreier generator being constructed
      a,       # new generator(s) of the current level, also scratch/group
      b,       # image(s) of `a' under `hom', also scratch/group
      relo,    # relative order (prime index) of an abelian comp. factor
      free,    # free group for the current level
      img,     # image of an orbit point under an acting generator
      pos,     # position of `img' in `orb' (or fail if new)
      reps,    # reps[i]: words in frees[i] mapping block 1 to each block
      lreps,   # list of block representative words for the current level
      hom,     # homomorphism from G to the acting group
      leno,    # orbit length of the previous level, i.e. the block length
      p,       # position of the block currently processed in `orb'
      fgens,   # generators of `free'
      fcache,  # cache of free groups -- reuse
      myfree,  # free group fetcher
      tryup;   # function: try to find the stabilizer by stepping upwards

  # Given the position <pos> of a point in the orbit of level <lev>, return
  # an element of G that maps this point back to <pnt>, by peeling off one
  # level at a time.
  invrep:=function(pos,lev)
  local elm,  # the group element constructed so far
        p,    # number of the block of level <lev> containing the point
        rep,  # word mapping block 1 to block p (to be inverted)
        a,    # value of `rep^-1' in G
        b;    # value of `rep^-1' in the acting group
    elm:=One(G);
    # go down through levels
    while lev>1 do
      if rlens[lev]>1 then # otherwise there was no growth
        p:=QuoInt(pos-1,lens[lev-1])+1; # number of block image
        rep:=reps[lev][p]^-1; # word for mapper back
        a:=MappedWord(rep,GeneratorsOfGroup(frees[lev]),levgens[lev]);
        b:=MappedWord(rep,GeneratorsOfGroup(frees[lev]),levacts[lev]);
        elm:=elm*a;
        pos:=act(orb[pos],b);
        pos:=LookupDictionary(d,pos); # mapped back
      fi;
      lev:=lev-1;
    od;
    Assert(1,pos=1); # the point must have been mapped back to <pnt>
    return elm;
  end;

  # Try to enlarge the stabilizer <s> within cs[i] by adjoining normalizing
  # elements of prime power order. Returns the enlarged group, or `fail' if
  # there are too many candidates for this to pay off.
  tryup:=function(s,i)
  local n,     # normalizer of <s> in cs[i]
        map,   # natural homomorphism n -> n/s
        q,     # the factor group n/s
        prev,  # image of the part of n below cs[i-1] (already treated)
        c,     # class representatives of q of prime power order
        zup,   # the relevant zuppos (prime power order elements) of q
        o,     # order of the class representative `a'
        cn,    # normalizer of <a> in q (to form the class of `a')
        t,     # loop variable over a transversal of cn in q
        u,     # the stabilizer as extended so far
        a;     # loop variable over class representatives resp. zuppos
    n:=Normalizer(cs[i],s);
    map:=NaturalHomomorphismByNormalSubgroup(n,s);
    q:=Image(map,n);
    prev:=Image(map,Intersection(n,cs[i-1]));
    # relevant Zuppos
    c:=List(ConjugacyClasses(q),Representative);
    c:=Filtered(c,x->IsPrimePowerInt(Order(x)) and not x in prev);
    zup:=[];
    for a in c do
      o:=Order(a);
      if ForAll([2..o],x->Gcd(o,x)<>1 or not a^x in zup) then
        cn:=Normalizer(q,Subgroup(q,[a]));
        for t in RightTransversal(q,cn) do
          Add(zup,a^t);
        od;
      fi;
      if Length(zup)>Length(orb) then
        return fail; # too many
      fi;
    od;
    Info(InfoAction,1,
      "try upstep ",i,": ",Size(q),"; ",Length(zup)," zuppos");

    # now try expanding with all possible normalizing zuppos
    u:=s;
    for a in zup do
      a:=PreImagesRepresentative(map,a);
      if not a in u then
        if act(pnt,ImagesRepresentative(hom,a))=pnt then
          u:=ClosureSubgroup(u,a);
        fi;
      fi;
    od;
    Info(InfoAction,1,"Found extra ",Size(u)/Size(s));
    return u;
  end;

  # reuse the same free groups again and again
  fcache:=[];
  myfree:=function(n)
    if not IsBound(fcache[n]) then
      fcache[n]:=FreeGroup(n);
    fi;
    return fcache[n];
  end;

  b:=ClosureSubgroup(RadicalGroup(G),PerfectResiduum(G));
  cs:=Reversed(CompositionSeriesThrough(G,[RadicalGroup(G),b]));

  d:=NewDictionary(pnt,true,dom);
  if gens=acts then
    hom:=IdentityMapping(G);
  else
    hom:=GroupGeneralMappingByImagesNC(G,Group(acts),gens,acts);
  fi;
  orb:=[pnt];
  ...
  for i in [2..Position(cs,b)] do
    Info(InfoAction,1,"Step ",i,": ",Length(orb)," Stb: ",Size(stb),
    " Group ",Index(cs[i],cs[i-1]));
    leno:=lens[i-1];

    # do we want to explore through normalizers up?
    if Length(orb)>10^4 and Index(G,cs[i])<Length(orb) then
      a:=stb;
      j:=i;
      while j<=Length(cs) and a<>fail do
        a:=tryup(a,j);
        j:=j+1;
      od;
      if a<>fail then
    ...
    if HasAbelianFactorGroup(cs[i],cs[i-1]) then
      Add(ablev,true);
      # standard pc stabilizer
      # the new generator
      a:=First(GeneratorsOfGroup(cs[i]),x->not x in cs[i-1]);
      b:=ImagesRepresentative(hom,a);
      relo:=Size(cs[i])/Size(cs[i-1]);
      Add(levgens,[a]);
      Add(levacts,[b]);
      free:=myfree(1); # reuse free groups
      Add(frees,free);
      img:=act(pnt,b);
      pos:=LookupDictionary(d,img);
      if pos=fail then
        Add(reps,List([0..relo-1],x->free.1^x));
        Add(rlens,relo);
        # move orbit as block
        Add(orb,img);
        ...
        od;
        for k in [3..relo] do
          for j in [(k-2)*leno+1..(k-1)*leno] do
            img:=act(orb[j],b);
            Add(orb,img);
            AddDictionary(d,img,Length(orb));
          od;
        ...
        # generator fixes orbit, add stabilizing element
        Add(rlens,1);
        Add(reps,0);
        b:=a*invrep(pos,i-1);
        Assert(1,act(pnt,ImagesRepresentative(hom,b))=pnt); # b stabilizes
        stb:=ClosureSubgroup(stb,b);
      fi;

    else
      # nonabelian part
      Add(ablev,false);
      a:=Filtered(GeneratorsOfGroup(cs[i]),x->not x in cs[i-1]);
      if ClosureSubgroup(cs[i-1],a{[1,2]})<>cs[i] then
        # pull back small gens
        b:=NaturalHomomorphismByNormalSubgroup(cs[i],cs[i-1]);
        a:=SmallGeneratingSet(Image(b,cs[i]));
        a:=List(a,x->PreImagesRepresentative(b,x));
      fi;
      b:=List(a,x->ImagesRepresentative(hom,x));
      Add(levgens,a);
      Add(levacts,b);
      free:=myfree(Length(a));
      fgens:=GeneratorsOfGroup(free);
      Add(frees,free);
      # now run an orbit algorithm on orbits
      lreps:=[One(free)];
      Add(reps,lreps);
      p:=1;
      while p<=Length(orb) do
        for j in [1..Length(a)] do
          ...
          if pos=fail then
            # new orbit image
            Add(lreps,lreps[QuoInt(p,leno)+1]*fgens[j]);
            Add(orb,img);
            AddDictionary(d,img,Length(orb));
            for k in [1..leno-1] do
              img:=act(orb[p+k],b[j]);
              Add(orb,img);
              AddDictionary(d,img,Length(orb));
            od;
            ...
            # stabilize
            k:=lreps[QuoInt(p,leno)+1]*fgens[j]/lreps[QuoInt(pos,leno)+1];
            k:=MappedWord(k,fgens,a); # stabilizes orbit
            pos:=LookupDictionary(d,act(pnt,ImagesRepresentative(hom,k)));
            k:=k*invrep(pos,i-1);
            stb:=ClosureSubgroup(stb,k);
          fi;
        od;
        p:=p+leno;
        if Length(orb)*Size(stb)=Size(cs[i]) then
          Info(InfoAction,2,"early break ",QuoInt(p,leno)+1," of ",Length(lreps));
          p:=Length(orb)+1;
        fi;
      od;
      Add(rlens,Length(lreps));

    fi;
    # orbit-stabilizer theorem must hold for the group treated so far
    Assert(2,Length(orb)*Size(stb)=Size(cs[i]));
