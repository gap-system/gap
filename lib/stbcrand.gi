...
#############################################################################
##
#F  StabChainRandomPermGroup(<G>,<options>) . . . . . .  random Schreier-Sims
##
...
InstallGlobalFunction( StabChainRandomPermGroup, function(S,options)
    ...
    S:=rec( generators := ShallowCopy(S), identity := () );
...
InstallGlobalFunction( VerifyStabilizer, function(S,z,missing,correct)
 ...
 if result = () then  
    ...
    for i in S.orbit do 
        if result = () and (not where2[i]) then 
           orb := [i]; 
           where2[i] := true; 
           for pnt  in orb  do
...
InstallGlobalFunction( VerifySGS, function(S,missing,correct) 
 ...
 while i < len  and  result = ()  do
     ...
     while (gencount < Length(list[len - i].generators)) and (result= () ) do
         gencount := gencount + 1;
         ...
         # obtained when adding gen
            ...
            if i=1 and Length(set) =1 then 
                ...
                if result <> () and temp2.orbit[1] > n then
