...
#1
##  Mathematically, a collection is a subset of a family.
...
##
##  is a common supercategory for `IsList' and `IsCollection' and can be
##  used for declarations that are applicable to both.
##
...
##
##  test whether an object is a collection.
DeclareCategory( "IsCollection", IsListOrCollection );


#############################################################################
##
#C  IsCollectionFamily(<Fam>) . . . test if an object is a collections family
##
...
##
##  returns a family for collections of objects in the family <F>.
##
...
##
##  returns the family from which the collections family <F> was created.
...
##
##  For the categories that are implied for any object in a family <Fam>,
##  by construction of <Fam>, the collections categories are regarded
##  as implied filters for any object in the collections family of <Fam>,
##  *provided that these collections categories exist already*.
...
##
BIND_GLOBAL( "InstallSubsetMaintainedMethod",
...
##
BIND_GLOBAL( "InstallIsomorphismMaintainedMethod",
...
##  collection in the filter <denom_req>,
...
##  are used in the case of isomorphisms.
##  So they should *not* be installed also as isomorphism maintained methods.
##
BIND_GLOBAL( "InstallFactorMaintainedMethod",
...
##  and `fail' otherwise.
##
DeclareOperation( "NextIterator", [ IsIterator ] );
...
##
##  `IsEmpty'  returns `true'  if the  collection  <C> is  empty (that is it
...
InstallIsomorphismMaintainedMethod( Size, IsCollection, IsCollection );
...
##  to fetch an element from known generators lists of <C>.
##  Note that `Representative' does not try to *compute* generators of <C>,
##  thus `Representative' may give up if <C> has no  generators at all.
##
...
##
##  returns the smallest (with respect to the $\<$ ordering) element in the
##  collection <C>. While the operation defaults to comparing all elements,
...
##
##  `Random'     returns   a random element    of    the collection <C>.  The
##  distribution of elements returned by `Random' depends on <C>.  For finite
##  collections all  elements  are  usually  equally likely.   For   infinite
##  collections  some reasonable distribution  is used.   See the chapters of
##  the various collections to find out which distribution is being used.
##  For some collections ensuring a reasonable distribution can be
##  difficult and require substantial runtime. If speed at the cost of equal
##  distribution is desired, the operation `PseudoRandom' should be used
##  instead.
##
...
##  The   distribution of   elements  returned   by  `PseudoRandom' is    not
##  necessarily an equal even for finite collections.
##
...
##  `Size(<coll>)' and <list> contains the elements (and holes) of <coll>
##  in the same order.
...
##
DeclareAttribute( "AsListSorted", IsListOrCollection );
...
#############################################################################
##
#F  List( <coll> )  . . . . . . . . . . . .  list of elements of a collection
#F  List( <list>, <func> )
##
##  In the first usage, `List' returns a new mutable list <list>.
##  <coll> must be a collection or a list.
...
##
##  The second usage takes a list <list> and a function <func> and returns
##  a list <new> defined by $<new>[i]:=<func>(<list>[i])$.
##
...
##  `<list>[<pos>]' executes in constant time, and the size of <list> is
##  proportional to its length.
##  `ListSorted' is an operation. `Set' is simply a synonym for
##  `ListSorted'.
##
...
##
DeclareOperation( "ListSorted", [ IsListOrCollection ] );
DeclareSynonym( "Set", ListSorted );
...
#############################################################################
##
#F  Sum( <C> )  . . . . . . . . . . . . . sum of the elements of a collection
#F  Sum( <C>, <func> )  . . . . . . . . . . .  sum of images under a function
##
...
#############################################################################
##
#F  Product( <C> )  . . . . . . . . . product of the elements of a collection
#F  Product( <C>, <func> )  . . . . . . .  product of images under a function
##
...
##
##  returns a collection containing the elements of <C> for which the
...
##
##  returns the number of elements of <C> for which the
##  function <func> returns `true'.
##
#O  NumberOp( <C>, <func> )
##
...
#############################################################################
##
#F  ForAll(<C>,<func>)  . . . . .  test a property for all elements of a list
##
##  tests whether the function <func> returns true for all elements in <C>.
##
#O  ForAllOp( <C>, <func> )
##
...
#############################################################################
##
#F  ForAny(<C>,<func>)  . . . . . . test a property for any element of a list
##
##  tests whether the function <func> returns true for at least one element
##  in <C>.
##
#O  ForAnyOp( <C>, <func> )
##
