...
#############################################################################
##
#F  StringOfUnivariateRationalPolynomialByCoefficients(
#F      <coeffs>, <nam> )
##
...
##
StringOfUnivariateRationalPolynomialByCoefficients := function( coeffs, nam )
    local string, i;
...
MolienSeries := function( arg )
    ...
    while not IsEmpty( classes ) do
      ...
      numer:= CoefficientsOfUnivariateLaurentPolynomial( summand.numer );
      a:= summand.a;
      ...
      numer:= List( ShiftedCoeffs( numer[1], numer[2] ),
                    y -> Trace( f, y ) )
              * ( Length( sub ) / Phi(n) );
      numer:= UnivariatePolynomial( Rationals, numer );
      ...
      if special <> false then
        ...
        if special[1] * special[2]
           < DegreeOfUnivariateLaurentPolynomial( num ) then
      ...
      while pos <= n do
        ...
        if a[ pos ] <> 0 then
          ...
          # Reduce the numerators if possible.
          num:= numer * p;
          if DegreeOfUnivariateLaurentPolynomial( num ) > pos * a[ pos ] then
    ...
    for i in [ 1 .. Length( numers ) ] do
      num:= CoefficientsOfUnivariateLaurentPolynomial( numers[i] );
    ...
    pol:= CoefficientsOfUnivariateLaurentPolynomial( pol );
    ...
    numerstring:= StringOfUnivariateRationalPolynomialByCoefficients(
        CoefficientsOfUnivariateLaurentPolynomial( numer )[1], "z" );
