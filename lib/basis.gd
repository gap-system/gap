...
##    (The call 'BasisDomain( <V> )' may return such a basis.)
...
##  as in FBA).
##
##  A mutable basis of a free left module is an object in 'IsMutable'
...
##  (whose methods have to guarantee consistency), and for that one can
##  eventually get an immutable basis with same basis vectors by 'Immutable'.
#T ??
##  
##  Since mutable bases do not admit arbitrary changes of their lists of
##  basis vectors, a mutable basis is *not* a list.
##  The family of a mutable basis is the family of its collection of
##  basis vectors.
##
