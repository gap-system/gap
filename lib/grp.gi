...
InstallFactorTrueMethod( IsPerfectGroup, IsGroup and IsPerfectGroup, IsGroup,
    IsGroup );
...
InstallMethod( JenningsSeries,
    ...
    while not IsTrivial( L[Length(L)] )  do
        ...
        L[n] := ClosureGroup( C, AgemoAbove( L[i], C, p ) );
    od;
...
InstallMethod( NormalClosure,
    ...
    Info( InfoGroup, 1,
          "NormalClosure: of ", GroupString(U,"U"), " in ",
          GroupString(G,"G") );
...
#F  SubgroupNC( <G>, <gens> )
##
##  Note that 'SubgroupNC' is allowed to call 'Objectify' in the case that
##  'gens' is empty.
##
Subgroup := function( G, gens )
    ...
    if not IsHomogeneousList(gens)  then
        Error( "<gens> must be a homogeneous list of elements" );
    fi;
    if not IsIdentical( FamilyObj(G), FamilyObj(gens) )  then
        Error( "families of <gens> and <grp> are different" );
    fi;
    if not ForAll( gens, x -> x in G  )  then
        Error( "<gens> must be elements in <G>" );
    fi;
    return SubgroupNC( G, gens );
end;


SubgroupNC := function( G, gens )
    local K, S;

    if IsEmpty( gens ) then
      K:= NewKind( FamilyObj(G),
                       IsGroup
                   and IsTrivial
