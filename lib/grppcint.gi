...
#F  ExtendedIntersectionSumPcgs( <pcgs>, <pcgsN>, <pcgsU> )
##
ExtendedIntersectionSumPcgs := function( pcgs, pcgsN, pcgsU )
    local   id,  G,  ls,  rs,  is,  i,  g,  I,  al,  ar,  z,  tmp,  
            sum,  intersection,  leftSide,  rightSide;
    ...
    od;
    for g in pcgsU do
        ls[ DepthOfPcElement( pcgs, g ) ] := g;
        rs[ DepthOfPcElement( pcgs, g ) ] := g;
    od;
    I := [];
    for g in pcgsN do
        if ls[ DepthOfPcElement( pcgs, g ) ] = id  then
            ls[ DepthOfPcElement( pcgs, g ) ] := g;
        else
            Add( I, g );
        fi;
    ...
    for al  in I  do
        ...
        while al <> id and ls[ z ] <> id  do
            tmp := LeadingExponentOfPcElement( pcgs, al )
                   ...
                   mod RelativeOrderOfPcElement( pcgs, al );
    ...
    intersection := InducedPcgsByPcSequence( pcgs,
                        Filtered( is, x -> x <> id ) );
...
#############################################################################
##
#F  SumFactorizationFunctionPcgs( <pcgs>, <pcgsU>, <pcgsN> )
##
SumFactorizationFunctionPcgs := function( pcgs, pcgsU, pcgsN )
    local   id,  S,  f;

    id := OneOfPcgs( pcgs );
    S  := ExtendedIntersectionSumPcgs( pcgs, pcgsN, pcgsU );
