#############################################################################
##
#W  lattice.gi                  GAP library                     Thomas Breuer
##
#H  @(#)$Id$
##
...
LinearIndependentColumns := function( mat )
    ...
    for k in [1..n]  do
        ...
        if j <= m  then
            ...
            # Normalize column 'k'.
            MultRowVector( mat[q], [ j .. m ], mat[q], [ j .. m ],
                           Inverse( row[k] ) );
    #         val:= row[k];
    #         for q in [ j .. m ] do
    #           mat[q][k] := mat[q][k] / val;
...
LLLint := function( lat )
  ...
  d:=[1,b[1]*b[1]];
  n:=Length(lat);
  Info( InfoLattice, 1, "integer LLL in dimension ", n );
  dim:=1;
  k:=2;
  ...
  while dim<n do
    ...
    Info( InfoLattice, 1, "computing vector ", dim );
