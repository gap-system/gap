...
##
## Hash dictionaries: This is essentially Frank Luebeck's code.
##

# initialize hash table: input lenght and hash function (giving a number)
BindGlobal("NaiveHashDictionary",function(domain,look,hashfun)
local full,ht,rep,len;
  if not HasSize(domain) then
    len:=1000;
  elif Size(domain)<100 then
    len:=100;
  elif Size(domain)<10000 then
    len:=Size(domain);
  else
    len:=100000;
  fi;
  full := QuoInt(len*9, 10);
  ht:=rec(len := len,    # length of table 
             full := full,  # we enlarge table, when filled by 90%
             nr := 0,       # current number of entries 
             els := [],     # stored elements 
             hashfun := hashfun);   # hash function
  if look then
    ht.vals := [];    # corresponding values (if wanted) 
    rep:=IsNaiveHashLookupDictionary;
  else
    rep:=IsNaiveHashDictionary;
  fi;
  Objectify(NewType(DictionariesFamily,rep),ht);
  return ht;
end);

# enlarge table by factor 2
BindGlobal("EnlargeHTDict",function(ht)
local len,full,oldels,oldvals,i;
  len:=ht!.len;
  oldels:=ht!.els;
  full := QuoInt(2*len*9, 10);
  ht!.len:=2*len;
  ht!.full:=full;
  ht!.nr:=0;
  ht!.els:=[];
  if IsNaiveHashLookupDictionary(ht) then
    oldvals:=ht!.vals;
    ht!.vals:=[];
    for i in [1..len] do
      if IsBound(oldels[i]) then
	AddDictionary(ht, oldels[i], oldvals[i]);
      fi;
    od;
  else
    for i in [1..len] do
      if IsBound(oldels[i]) then
	AddDictionary(ht, oldels[i]);
      fi;
    od;
  fi;
end);
...
#M  AddDictionary(<dict>,<obj>,<val>)
##
InstallOtherMethod(AddDictionary,"for lookup hash dictionaries",true,
  [IsNaiveHashLookupDictionary,IsObject,IsObject],0,
function(ht, x, val)
local  h,l,i,a;
  if ht!.full = ht!.nr+1 then
    EnlargeHTDict(ht);
  fi;
  l:=ht!.len;
  h := ht!.hashfun(x) mod l + 1;
...
#############################################################################
##
#M  AddDictionary(<dict>,<obj>)
##
InstallOtherMethod(AddDictionary,"for hash dictionaries, no value given",true,
  [IsNaiveHashDictionary,IsObject],0,
function(ht, x)
local  h,l,i,a;
  if ht!.full = ht!.nr+1 then
    EnlargeHTDict(ht);
...
#M  KnowsDictionary(<dict>,<obj>)
##
InstallMethod(KnowsDictionary,"for hash dictionaries",true,
  [IsNaiveHashDictionary,IsObject],0,
function(ht, x)
local   h,l;
  l:=ht!.len;
  h := ht!.hashfun(x) mod l + 1;
  while IsBound(ht!.els[h]) and ht!.els[h] <> x do
    h := h+1;
    if h>l then h:=1; fi;
  od;
...
#M  LookupDictionary(<dict>,<obj>)
##
InstallMethod(LookupDictionary,"for hash lookup dictionaries",true,
  [IsNaiveHashLookupDictionary,IsObject],0,
function(ht, x)
local   h,l;
  l:=ht!.len;
  h := ht!.hashfun(x) mod l + 1;
