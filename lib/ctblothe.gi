...
BindGlobal( "MOCTable0", function( gaptbl )
    ...
    gaptbl_powermap:= ShallowCopy( ComputedPowerMaps( gaptbl ) );
    ...
    for p in [ 2 .. Maximum( Maximum( gaptbl_orders ), 19 ) ] do
      if not IsBound( gaptbl_powermap[p] ) and IsPrimeInt( p ) then
        gaptbl_powermap[p]:= PowerMap( gaptbl, p );
      fi;
    od;
    ...
    for p in [ 2, 3, 5, 7, 11, 13, 17, 19 ] do
      result.powerinfo[p]:=
                MOCPowerInfo( extendedfields, gal, gaptbl_powermap, p );
    od;
...
InstallGlobalFunction( MOCString, function( arg )
    ...
    if Length( arg ) = 1 and IsMatrix( arg[1] ) then
      ...
      Error( "usage: PrintToMOC( <moctbl>[, <chars>] )" );
    else
