#@local BruteTail,G,Invariant,M,N,P,U,elms,hom,iso,p,pcgs
...
3
gap> Invariant := G -> [ Size(G), Collected(List(ConjugacyClasses(G),
>      c -> [ Order(Representative(c)), Size(c) ])) ];;
gap> Invariant(PcGroupWithPcgs(pcgs)) = Invariant(SymmetricGroup(4));
true
