...
#W                                                               Frank Celler
##
#H  @(#)$Id: testutil.g,v 4.31 2011/05/06 23:53:08 alexk Exp $
##
...
Revision.( "tst/testutil_g" ) :=
    "@(#)$Id: testutil.g,v 4.31 2011/05/06 23:53:08 alexk Exp $";
...
BindGlobal( "CreatePackageLoadTestsInput", 
    ...
    local mode, opt, result, name, entry, packagenames;
    ...
    for name in packagenames do
        for entry in GAPInfo.PackagesInfo.( name ) do
            ...
            Append( result, 
                Concatenation( "echo 'LoadPackage( \"", name, "\"", opt, ");",
    ...
    od;
    Append( result, "echo '==========================================='\n" );
    Append( result, Concatenation("echo 'Testing LoadAllPackages", mode, "'\n" ) );
    Append( result, 
        Concatenation( "echo 'if CompareVersionNumbers( GAPInfo.Version, \"4.5.0\") then ",
                                "SetInfoLevel(InfoPackageLoading,4);",
                                "fi;LoadAllPackages(", opt, ");' | ", 
                       gap, " > ", outfileprefix, "$TIMESTAMP.all 2>&1 \n" ) );
    Append( result, 
        Concatenation( "cat ", outfileprefix, "$TIMESTAMP.all\n" ) );
    Append( result, "echo '==========================================='\n" );
    Append( result, Concatenation("echo 'Testing LoadAllPackages in the reverse order", mode, "'\n" ) );
    if opt="" then
      	opt:=":reversed";
    else
      	opt:=":OnlyNeeded,reversed";
    fi;
    Append( result, 
        Concatenation( "echo 'if CompareVersionNumbers( GAPInfo.Version, \"4.5.0\") then ",
                                "SetInfoLevel(InfoPackageLoading,4);",
